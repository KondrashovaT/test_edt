...
Процедура ШтатноеРасписаниеФормаСпискаПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт 
	...
	Форма.СтруктураПредприятияСписок.ТекстЗапроса =
		...
		|	СтруктураПредприятия.Сформировано КАК Сформировано,
		|	СтруктураПредприятия.ДатаСоздания КАК ДатаСоздания,
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.35.69";
	Обработчик.Процедура = "ОрганизационнаяСтруктура.ЗаполнитьДатыСозданияВСтруктуреПредприятия";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("656a4c7c-ff44-4395-869e-aa1aacd1d6ae");
	Обработчик.Комментарий = НСтр("ru = 'Заполнение дат создания элементов справочника «Структура предприятия».'");	
...
Процедура ЗаполнитьДатыСозданияВСтруктуреПредприятия(ПараметрыОбновления = Неопределено) Экспорт
	...
	ЭлементБлокировки = Блокировка.Добавить("Справочник.СтруктураПредприятия");
	ЭлементБлокировки.ИсточникДанных = ВышестоящиеПодразделения(ПодразделенияКОбработке);
	...
	Если ГруппаОтборов <> Неопределено Тогда
		...
		Если ОрганизационнаяСтруктураСтраница = Неопределено Тогда
			...
			Форма.ОрганизационнаяСтруктура.ТекстЗапроса =
				...
				|	КОНЕЦ КАК СформированоНаСегодня,
				|	ВЫБОР
				...
				|	КОНЕЦ КАК РасформированоНаСегодня
				|ИЗ
				|	Справочник.СтруктураПредприятия КАК СтруктураПредприятия";
...
// Параметры:
//   ПараметрыОбновления - см. УправлениеШтатнымРасписанием.ПараметрыОбновленияСведенийПодразделений
Функция ЗаполнитьПараметрыОбновленияСведенийПодразделений(ПараметрыОбновления)
	
	ЗапросВТ = ЗапросВТДатыОкончанияМестаПозицииВСтруктуреПредприятия();
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьИсториюИзмененияШтатногоРасписания") Тогда
		...
		ПодстрокиЗамены.ТаблицаШтатноеРасписаниеРасширенная =
			...
			|	ПО ПодчиненностьПодразделений.СтруктурнаяЕдиница = МестоПозицииШтатногоРасписанияВСтруктуреПредприятия.Подразделение
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияИспользованияШтатногоРасписания";
		ПодстрокиЗамены.УсловиеСоединения =
			"МестоПозицииШтатногоРасписанияВСтруктуреПредприятия.Позиция = ШтатноеРасписание.ПозицияШтатногоРасписания";
		...
		ПодстрокиЗамены.ПолеДата = "ВЫБОР
		...
		ПодстрокиЗамены.ПолеМаксимальнаяДата = СтрШаблон("ВЫБОР
			...
			|			ИНАЧЕ %1
			|		КОНЕЦ", ПодстрокиЗамены.ПолеМаксимальнаяДатаИзменений);
	...
	Иначе
		...
		ПодстрокиЗамены.ПолеДата = "ВЫБОР
			|			КОГДА МестоПозицииШтатногоРасписанияВСтруктуреПредприятия.ДатаНачала > ШтатноеРасписание.ДатаУтверждения
			|				ТОГДА МестоПозицииШтатногоРасписанияВСтруктуреПредприятия.ДатаНачала
			|			ИНАЧЕ ШтатноеРасписание.ДатаУтверждения
			|		КОНЕЦ";
		ПодстрокиЗамены.ПолеМаксимальнаяДатаИзменений = "ВЫБОР
