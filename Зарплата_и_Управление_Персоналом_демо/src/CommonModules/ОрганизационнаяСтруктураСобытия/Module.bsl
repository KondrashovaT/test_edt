...
Функция ИзмененныеЭлементыИсточника(Источник)
	...
	ТекстЗапроса = 
		...
		|	СтруктураПредприятия.ДатаСоздания КАК ДатаСозданияЭлементаСтруктуры,
		|	ТаблицаИсточника.Сформировано КАК Сформировано,
		|	ТаблицаИсточника.ДатаСоздания КАК ДатаСоздания,
		|	ТаблицаИсточника.Расформировано КАК Расформировано,
		|	ТаблицаИсточника.ДатаРасформирования КАК ДатаРасформирования
		|ИЗ
		|	#СтруктураИсточник КАК ТаблицаИсточника
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтруктураПредприятия КАК СтруктураПредприятия
		|		ПО (СтруктураПредприятия.Источник = ТаблицаИсточника.Ссылка)
		...
		|			ИЛИ &РеквизитСформированоИзменился
		|			ИЛИ &РеквизитДатаСозданияИзменился
		|			ИЛИ &РеквизитРасформированоИзменился
		|			ИЛИ &РеквизитДатаРасформированияИзменился)";
	...
	ИзменитьЗапросНаОснованииРеквизита(Запрос, МетаданныеИсточника.Реквизиты, "Сформировано", ТекстЗапроса);
	ИзменитьЗапросНаОснованииРеквизита(Запрос, МетаданныеИсточника.Реквизиты, "ДатаСоздания", ТекстЗапроса);
	ИзменитьЗапросНаОснованииРеквизита(Запрос, МетаданныеИсточника.Реквизиты, "Расформировано", ТекстЗапроса);
	ИзменитьЗапросНаОснованииРеквизита(Запрос, МетаданныеИсточника.Реквизиты, "ДатаРасформирования", ТекстЗапроса);
	
	Запрос.Текст = ТекстЗапроса;
	
	ИзмененныеЭлементы = Запрос.Выполнить().Выгрузить();
...
Процедура ИзменитьЗапросНаОснованииРеквизита(Знач Запрос, Знач РеквизитыИсточника, Знач ИмяРеквизита, ТекстЗапроса)
	
	Если РеквизитыИсточника.Найти(ИмяРеквизита) = Неопределено Тогда
		Если ИмяРеквизита = "Сформировано" Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаИсточника.Сформировано", "ИСТИНА");
			ТекстЗапроса = СтрЗаменить(
				ТекстЗапроса, "&РеквизитСформированоИзменился", "НЕ СтруктураПредприятия.Сформировано");
		ИначеЕсли ИмяРеквизита = "ДатаСоздания" Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаИсточника.ДатаСоздания", "&ПустаяДата");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
				"&РеквизитДатаСозданияИзменился",
				"СтруктураПредприятия.Сформировано И СтруктураПредприятия.ДатаСоздания = &ПустаяДата");
		Иначе
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
				СтрШаблон("ТаблицаИсточника.%1", ИмяРеквизита),
				"НЕОПРЕДЕЛЕНО");
		КонецЕсли;
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			СтрШаблон("&Реквизит%1Изменился", ИмяРеквизита),
			СтрШаблон("СтруктураПредприятия.%1 <> ТаблицаИсточника.%1", ИмяРеквизита));
	КонецЕсли;
	Запрос.УстановитьПараметр(СтрШаблон("Реквизит%1Изменился", ИмяРеквизита), Ложь);
