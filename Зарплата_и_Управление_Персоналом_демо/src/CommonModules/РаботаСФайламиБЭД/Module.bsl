...
Функция РаспаковатьАрхивСИзменениемИменФайлов(ДвоичныеДанныеАрхива, КаталогРаспаковки, КонтекстДиагностики) Экспорт
	...
	Для Каждого ЭлементZip Из ЧтениеZip.Элементы Цикл
		...
		СоответствиеИменФайлов.Вставить(ЭлементZip.Имя, ДанныеФайла);
		СоответствиеИменФайлов.Вставить(ЭлементZip.ИсходноеИмя, ДанныеФайла);
	КонецЦикла;
	ЧтениеZip.Закрыть();
	...
	Для Каждого Файл Из МассивФайлов Цикл
		...
		Для Каждого КлючИЗначение Из СоответствиеИменФайлов Цикл
			Если КлючИЗначение.Значение.ИмяФайла = Файл.Имя Тогда
				ДанныеФайла.Имя = КлючИЗначение.Ключ;
...
Функция БуферИмениИзСоответствия(БуферИмени, СоответствиеИменФайлов)
	...
	Для Каждого Кодировка Из ВозможныеКодировки Цикл
		...
		Если СтрНайти(ИмяФайлаВКодировке, "/") > 0 Тогда
			ПутьИИмя = СтрРазделить(ИмяФайлаВКодировке, "/", Ложь);
			ИмяФайлаВКодировке = ПутьИИмя.Получить(ПутьИИмя.Количество()-1);
		ИначеЕсли СтрНайти(ИмяФайлаВКодировке, "\") > 0 Тогда
			ПутьИИмя = СтрРазделить(ИмяФайлаВКодировке, "\", Ложь);
			ИмяФайлаВКодировке = ПутьИИмя.Получить(ПутьИИмя.Количество()-1);
		КонецЕсли;
