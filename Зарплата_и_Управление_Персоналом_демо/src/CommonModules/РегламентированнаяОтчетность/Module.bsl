...
Функция СкопироватьУзел(Родитель, Узел) Экспорт 
	...
	// находим узел с максимальным индексом и с тем же кодом, что и у копируемого, и добавляем новый
	СтрокиСТемЖеКодом = Родитель.Строки.НайтиСтроки(Новый Структура("Код", Узел.Код), Ложь);
	
	Если Узел.Код = "row" Тогда
		
		Для Каждого СтрокаСТемЖеКодом Из СтрокиСТемЖеКодом Цикл
			Если СтрокаСТемЖеКодом = Узел Тогда
				ИндексУзла = Родитель.Строки.Индекс(СтрокаСТемЖеКодом);
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		НовыйУзел = Родитель.Строки.Вставить(ИндексУзла);
	...
	Иначе
		...
		Если ИндексСоздаваемогоУзла >= Родитель.Строки.Количество() ИЛИ ИндексСоздаваемогоУзла = 0 Тогда // если найденный узел - последний или аналогичный узел не найден
