...
Функция СводнаяИнформацияПоДнямУхода(Документ, Отказ = Ложь) Экспорт
	Перем ТекущаяСсылка, КонецМесяца, НачалоГода;
	...
	|	И ВТПлановоеВремя.ВидУчетаВремени = ЗНАЧЕНИЕ(Справочник.ВидыИспользованияРабочегоВремени.Явка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОплатаДнейУходаЗаДетьмиИнвалидами КАК ОплатаДнейУхода
	|		ПО ДниПоДокументам.Ссылка = ОплатаДнейУхода.Ссылка
	|			И (ОплатаДнейУхода.ИспользоватьОстатокДнейЗаГод)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ДниЗаГод КАК ДниЗаГод
	|ГДЕ
	|	ДниЗаГод.ИспользоватьОстатокДнейЗаГод
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО ВТПлановоеВремя.Дата = ВсеДни.Дата
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ИнтервалПоФизическомуЛицу КАК ИнтервалПоФизическомуЛицу
	|		ПО ВТПлановоеВремя.Дата >= ИнтервалПоФизическомуЛицу.ДатаНачала
	|			И ВТПлановоеВремя.Дата <= ИнтервалПоФизическомуЛицу.ДатаОкончания
	|ГДЕ
	|	ВТПлановоеВремя.ЧасыНорма > 0
	|	И ВТПлановоеВремя.ВидУчетаВремени = ЗНАЧЕНИЕ(Справочник.ВидыИспользованияРабочегоВремени.Явка)
	|	И ВсеДни.Дата ЕСТЬ NULL
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата
	|;
	...
	СтруктураРезультат.ПериодБезРазрыва = Пакет[ВсегоПакетов - 7].Выгрузить().Количество() = 0;
	СтруктураРезультат.ВыходныеДни = Пакет[ВсегоПакетов - 6].Выгрузить().ВыгрузитьКолонку("Дата");
	...
	ИспользоватьОстатокДнейЗаГод = СтруктураРезультат.ИспользоватьОстатокДнейЗаГод;
	СтруктураРезультат.ИспользоватьОстатокДнейЗаГод = СтруктураРезультат.КоличествоРабочихДней > 4;
	
	Если СтруктураРезультат.КоличествоКалендарныхДней <= 4 Тогда
		// Проверка на излишне взведенный флаг в других документах при пересечении дат с текущим.
		ВыборкаСсылка = Пакет[ВсегоПакетов - 10].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаСсылка.Следующий() Цикл
			Если ВыборкаСсылка.ИспользоватьОстатокДнейЗаГод Тогда
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон(
					НСтр("ru = 'В документе %1 необходимо снять флаг ""Использовать остаток дней за год"".'"), ВыборкаСсылка.Ссылка),
	...
	Если СтруктураРезультат.КоличествоКалендарныхДней > 24 Тогда
		Текст = СтрШаблон(НСтр("ru = 'Использованно более 24 дней по физическому лицу - (%1).'"),
			ПредставлениеКоличестваДней(СтруктураРезультат.КоличествоКалендарныхДней));
		СообщенияБЗК.СообщитьОбОшибкеВОбъекте(Отказ, ТекущаяСсылка, Текст, "Объект.ДатаОкончания");
		Возврат СтруктураРезультат;
	КонецЕсли;
	...
	СтруктураРезультат.ИспользоватьОстатокДнейЗаГод = ЕстьПревышение;
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		Если ПустаяСтрока(СтруктураРезультат.ТекстПересечнияОстатка) Тогда
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Найдено пересечение дат (%1) с документом %2, необходимо взвести флаг ""Использовать остаток дней за год"" во всех документах.'"),
	...
	НеУчтенныеДниДокумента = Пакет[ВсегоПакетов - 8].Выгрузить().Итог("КоличествоДней");
	...
	// Проверка на разрыв дополнительных оплачиваемых выходных дней подряд в пределах
	// общего количества неиспользованных дополнительных оплачиваемых выходных дней (рабочих) в календарном году (максимум 24).
	Если ИспользоватьОстатокДнейЗаГод Или СтруктураРезультат.КоличествоРабочихДнейПоДокументу > 4 Тогда
		ДатыРазрыва = Новый Массив;
		ПериодыДокументов = Пакет[ВсегоПакетов - 9].Выгрузить();
		Для Каждого ДатаСотрудника Из Пакет[ВсегоПакетов - 5].Выгрузить() Цикл
			ЕстьРазрыв = Истина;
			Для Каждого ПериодДокумента Из ПериодыДокументов Цикл
				Если ДатаСотрудника.Сотрудник = ПериодДокумента.Сотрудник И ДатаСотрудника.Дата >= ПериодДокумента.ДатаНачала
					И ДатаСотрудника.Дата <= ПериодДокумента.ДатаОкончания Тогда
					Если ДатыРазрыва.Найти(ДатаСотрудника.Дата) = Неопределено Тогда
						ДатыРазрыва.Добавить(ДатаСотрудника.Дата);
					КонецЕсли;
					ЕстьРазрыв = Ложь;
					Прервать;
				ИначеЕсли ДатаСотрудника.Сотрудник <> ПериодДокумента.Сотрудник И ДатаСотрудника.Дата >= ПериодДокумента.ДатаНачала
					И ДатаСотрудника.Дата <= ПериодДокумента.ДатаОкончания Тогда
					ЕстьРазрыв = Ложь;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если ЕстьРазрыв И ДатыРазрыва.Найти(ДатаСотрудника.Дата) = Неопределено Тогда
				ДатыРазрыва.Добавить(ДатаСотрудника.Дата);
			КонецЕсли;
		КонецЦикла;
		ПроверитьЦелостностьПериода(ТекущаяСсылка, ДатыРазрыва, СтруктураРезультат, Отказ);
	КонецЕсли;
...
Процедура ПроверитьЦелостностьПериода(ТекущаяСсылка, МассивДат, СтруктураРезультат, Отказ)
	ПериодыРазрывов = ОбщегоНазначенияБЗК.ПериодыИзМассиваДат(МассивДат);
	Для каждого ПериодРазрыва Из ПериодыРазрывов Цикл
		Если НачалоДня(ПериодРазрыва.ДатаНачала) = НачалоДня(ПериодРазрыва.ДатаОкончания) Тогда
			ТекстСообщения = НСтр("ru = 'По физическому лицу прерывается период накопленного отпуска по уходу %1.'");
		...
		Иначе 				
			ТекстСообщения = НСтр("ru = 'По физическому лицу прерывается период накопленного отпуска по уходу с %1 по %2.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстСообщения, Формат(ПериодРазрыва.ДатаНачала,"ДЛФ=D"), Формат(ПериодРазрыва.ДатаОкончания,"ДЛФ=D"));
			КонецЕсли;
			СообщенияБЗК.СообщитьОбОшибкеВОбъекте(Отказ, ТекущаяСсылка, ТекстСообщения, "Объект.ДатаОкончания");
	КонецЦикла;
КонецПроцедуры
