...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение 
		...
		ФизическиеЛица = ОбщегоНазначенияБЗККлиентСервер.ВыгрузитьКолонку(Объект.Сотрудники, "ФизическоеЛицо", Истина);
		МатериальноОтветственныеЛицаКлиент.ПроверитьМатериальнуюОтветственность(Объект.Организация, ФизическиеЛица, ЭтаФорма, Отказ, ПараметрыЗаписи);
		Если Отказ Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
